...
// Para o mobile, achata tudo em lista plana
const flatNavItems = mainNavItems.flatMap((item): NavItem[] =>
    item.children ? item.children : [item]
);

const rightNavItems = footerNavItems;
</script>
                        ...
                        <NavigationMenuList class="flex h-full items-stretch space-x-1">
                            <template v-for="(item, index) in mainNavItems" :key="index">
                                ...
                                <NavigationMenuItem v-if="item.children" class="relative flex h-full items-center">
                                    ...
                                    <NavigationMenuContent>
                                        <ul class="w-48 p-1">
                                            <li v-for="child in item.children" :key="child.title">
                                                <Link
                                                    :href="child.href ?? '#'"
                                                    class="flex items-center gap-x-2 rounded-md px-3 py-2 text-sm font-medium hover:bg-accent"
                                    ...
                                    <!-- underline se qualquer filho estiver ativo -->
                                    <div
                                        v-if="item.children.some(c => isCurrentUrl(c.href ?? ''))"
